...
--Function: findata.getddpayments(Input insdatemin varchar, Input insdatemax varchar, Input inmsgtype varchar, Input insender varchar, Input inreceiver varchar, Input inref varchar, Input invdate varchar, Input inamtmin numeric, Input inamtmax numeric, Input inccy varchar, Input indacc varchar, Input indcname varchar, Input incdtid varchar, Input incacc varchar, Input inccname varchar, Input indirect varchar, Input instate varchar, Input inbatchid varchar, Input inuserid integer, Input inordfield varchar, Input inorddir varchar, Input inllimit integer, Input inulimit integer, Output outretcursor refcursor)

--DROP FUNCTION findata.getddpayments(IN insdatemin varchar, IN insdatemax varchar, IN inmsgtype varchar, IN insender varchar, IN inreceiver varchar, IN inref varchar, IN invdate varchar, IN inamtmin numeric, IN inamtmax numeric, IN inccy varchar, IN indacc varchar, IN indcname varchar, IN incdtid varchar, IN incacc varchar, IN inccname varchar, IN indirect varchar, IN instate varchar, IN inbatchid varchar, IN inuserid integer, IN inordfield varchar, IN inorddir varchar, IN inllimit integer, IN inulimit integer, OUT outretcursor "refcursor");

CREATE OR REPLACE FUNCTION findata.getddpayments
(
...
/************************************************
  ...
  Created:        28.May.2014, DenisaN 8425
  Description:    Gathers data for the Direct Debit reporting according to the given filtering     
...
'select * from ('||
 ...
 ' select  row_number() over (order by '||coalesce(inordfield, 'insertdate ')||' '||coalesce(inorddir, ' desc')||' ) rnum, '||   
         ...
         ' dbtaccount, dbtcustname, cdtid, cdtaccount, cdtcustname, '||
  ...
  ' where insertdate >= to_timestamp($1,''dd mm yyyy hh24:mi:ss'') and insertdate <= to_timestamp($2,''dd mm yyyy hh24:mi:ss'') '||
       ...
       ' and ( $21 is null or lower(cdtid) like lower(''%''||$22||''%'')) ' ||
       ' and ( $23 is null or lower(cdtaccount) like lower(''%''||$24||''%'')) ' ||
       ' and ( $25 is null or lower(cdtcustname) like lower(''%''||$26||''%'')) ' ||
       ' and ( $27 is null or direction = $28) ' ||
       ' and ( $29 is null or state = $30) ' ||
       ' and ( $31 is null or lower(batchid) like lower(''%''||$32||''%'')) ' ||
       ' and ( $33 is null or userid = $34 ) ' ||
  ' order by '||coalesce(inordfield, 'insertdate')||' '||coalesce(inorddir, 'desc')||') tmp ) tmp1'||
  ' where rnum > coalesce($35,0) and rnum <= coalesce($36,100)+ coalesce($37,0)'
using insdatemin, insdatemax, inmsgtype, inmsgtype, insender, insender, inreceiver, inreceiver, inref, inref,  invdate, invdate, inamtmin, inamtmax,
      inccy, inccy, indacc, indacc, indcname, indcname, incdtid, incdtid, incacc, incacc, inccname, inccname, 
...
ALTER FUNCTION findata.getddpayments(IN insdatemin varchar, IN insdatemax varchar, IN inmsgtype varchar, IN insender varchar, IN inreceiver varchar, IN inref varchar, IN invdate varchar, IN inamtmin numeric, IN inamtmax numeric, IN inccy varchar, IN indacc varchar, IN indcname varchar, IN incdtid varchar, IN incacc varchar, IN inccname varchar, IN indirect varchar, IN instate varchar, IN inbatchid varchar, IN inuserid integer, IN inordfield varchar, IN inorddir varchar, IN inllimit integer, IN inulimit integer, OUT outretcursor "refcursor")
  OWNER TO findata;

GRANT EXECUTE
  ON FUNCTION findata.getddpayments(IN insdatemin varchar, IN insdatemax varchar, IN inmsgtype varchar, IN insender varchar, IN inreceiver varchar, IN inref varchar, IN invdate varchar, IN inamtmin numeric, IN inamtmax numeric, IN inccy varchar, IN indacc varchar, IN indcname varchar, IN incdtid varchar, IN incacc varchar, IN inccname varchar, IN indirect varchar, IN instate varchar, IN inbatchid varchar, IN inuserid integer, IN inordfield varchar, IN inorddir varchar, IN inllimit integer, IN inulimit integer, OUT outretcursor "refcursor")
TO findata;
